def call(Map config = [:]) {
  ...
  if (!config.containsKey('initSSHCredentialId') || !(config.initSSHCredentialId instanceof CharSequence)) {
  ...
  if (!config.containsKey('initSSHHostKeys') || !(config.initSSHHostKeys instanceof List)) {
