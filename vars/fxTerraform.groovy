...
def call(Map config = [:], Map closures = [:]) {
  ...
  mapAttributeCheck(config, 'initSSHCredentialId', List, ['gitea-fx_administrator-key'])
...
private preValidate(Boolean deployFileExists, ScmInfo scmInfo) {
  if (!deployFileExists) {
    ...
    if (!(scmInfo.getRepositoryName() ==~ /^cloud-terraform\-(module|ecosystem)\-(aws|azurerm|azuread|google|bitbucket|gitlab|github|kubernetes|multi)-[a-z\d]{3,}([a-z\d\-]+)?$/)) {
